...
$contenido_correo = "<h2 style='text-align:center;background:#31b312;color:#FFF;border-radius:5px;padding:5px;'>".utf8_decode($data_nombre_curso)."</h2>";
$contenido_correo .= "<center><a href='".$dominio."$titulo_identificador_curso.html'><img style='width:230px;padding:1px;border:1px solid gray;border-radius:5px;background:#31b312;' src='".$dominio_www."contenido/alt/logotipo-v3.png'/></a></center>";
...
switch ($modo_envio) {
    case 'usuariosreg':
        ...
        $rqtur1 = query("SELECT DISTINCT email FROM cursos_usuarios WHERE estado=1 AND sw_notif=1 AND email LIKE '%@%' AND email NOT IN (select email from cursos_rel_notifcuremail where id_curso='$id_curso') AND email NOT IN (select email from cursos_unsubscribe_ustag where id_tag IN (SELECT id_tag FROM cursos_rel_cursostags WHERE id_curso='$id_curso')) $qr_ciudades_1 $qr_bloque ");
        $cnt_env = 0;
        while ($rqturap2 = fetch($rqtur1)) {
            $correo = $rqturap2['email'];
            $enlace_dar_baja_curso = '<a href="'.$dominio.'unsubscribe/' . $correo . '/' . $id_curso . '/' . md5($correo . 'dardebaja') . '.html">Ya tome este curso anteriormente</a>';
            ...
            if((int)$rqder2['total']==0){
                ...
                SISTsendEmail($correo, $asunto, $cont_preunsubscribe . $contenido_correo . $cont_unsubscribe);
                $cnt_env++;
                if ($cnt_env == 1) {
    ...
    case 'partnoasist':
        ...
        while ($rqturap2 = fetch($rqturapn1)) {
            ...
            $cont_unsubscribe = "<hr/><div style='text-align:center;'>$enlace_dar_baja_curso | $enlace_dar_baja_notificaciones</div>";
            query("INSERT INTO cursos_rel_notifcuremail (id_curso,email) VALUES ('$id_curso','$correo') ");
            SISTsendEmail($correo, $asunto, $contenido_correo . $cont_unsubscribe);
            $cnt_env++;
            if ($cnt_env == 1) {
    ...
    case 'pushnav':
        ...
        $arrNotificationMessage = array(
            ...
            'image' => $url_img_curso,
            'icon' => $url_img_curso,
            'click_action' => $dominio."$titulo_identificador_curso.html",
            'url' => $dominio."$titulo_identificador_curso.html",

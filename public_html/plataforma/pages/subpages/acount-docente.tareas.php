                        ...
                        while ($rqp2 = fetch($rqp1)) {
                            ...
                            <tr>
                                ...
                                <td class="text-center" id="ajaxbox-<?php echo $rqp2['id']; ?>">
                                    <?php
                                    $rqva1 = query("SELECT u.apellidos,u.nombres,e.id,e.archivo,e.calificacion FROM cursos_onlinecourse_tareasenvios e INNER JOIN cursos_usuarios u ON e.id_usuario=u.id WHERE e.id_tarea='".$rqp2['id']."' ORDER BY u.nombres ASC ");
                                    if (num_rows($rqva1) == 0) {
                                        ?>
                                        0 env&iacute;os
                                        <?php
                                ...
                                <td class="text-center" id="ajaxbox-<?php echo $rqp2['id']; ?>">
                                    ...
                                    <br>
                                    <b class="btn btn-xs btn-success btn-block" style="padding-top:2px;padding-bottom:2px;background: #0cb5a5;" onclick="editar_tarea(<?php echo $rqp2['id']; ?>)"><i class="fa fa-edit"></i> Editar tarea</b>
                                </td>
                            </tr>
                            <?php
                        }
...
<script>
    function envios_tarea(id_tarea) {
        ...
        $.ajax({
            url: 'pages/ajax/ajax.acount-docente.tareas.envios_tarea.php',
...
<script>
    function editar_tarea(id_tarea) {
        $("#title-MODAL-general").html('EDICI&Oacute;N DE TAREA');
        $("#body-MODAL-general").html('Cargando...');
        $("#MODAL-general").modal('show');
        $.ajax({
            url: 'pages/ajax/ajax.acount-docente.tareas.editar_tarea.php',

  ...
  <strong>AVISO IMPORTANTE!</strong><br>Lamentamos mucho la molestia, este curso se encuentra en mantenimiento durante 24 horas, por favor vuelve el d&iacute;a de ma&ntilde;ana.
                    ...
                    if (!$sw_acceso_a_curso) {
                        ...
                        if ($modo_leccion == 'presentacion') {
                            ...
                            if (num_rows($rqvs1) > 0) {
                                ?>
                                <iframe style="border:0px;width:100%;height:670px;" src="<?php echo $dominio_www; ?>contenido/paginas/procesos/presentacion.php?id_leccion=<?php echo $leccion['id']; ?>"></iframe>
                                <?php
                            } else {
                        ...
                        if ($modo_leccion == 'video') {
                            if ($leccion['video'] !== '' && $leccion['sw_vimeo']=='1') {
                                ...
                                $url_imagen_v = $dominio_www."contenido/imagenes/images/banner-general-cursosbo.jpg";
                                if ($curso['imagen'] !== '') {
                                    $url_imagen_v = $dominio_www.'paginas/' . $curso['imagen'] . '.size=4.img';
                                }
...
<script>
    function reg_avance() {
        $.ajax({
            url: 'pages/ajax/ajax.curso-online.leccion.reg_avance.php',
            data: {id_onlinecourse: '<?php echo $id_onlinecourse; ?>', id_onlinecourse_leccion: '<?php echo $id_onlinecourse_leccion; ?>'},
            type: 'POST',
            dataType: 'json',

...
<div class="row">
    <div class="col-mod-12">
        <ul class="breadcrumb">
            ...
            <li class="active">LOGIN LOG (loadpage)</li>
        </ul>
            ...
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            <?php
                            $resultado1 = query("SELECT *,(select nombre from administradores ad where l.id_administrador=ad.id limit 1)administrador FROM login_log l ORDER BY id DESC LIMIT $start,$registros_a_mostrar ");
                            $resultado1b = query("SELECT count(*) AS total FROM login_log l ");
                            $resultado2b = mysql_fetch_array($resultado1b);
                            $total_registros = $cnt = $resultado2b['total'] - (($vista - 1) * $registros_a_mostrar);
        ...
        <div class="row">
            <div class="col-md-12">
                <ul class="pagination">
                    ...
                    <li><a <?php echo loadpage('login-log/1'); ?>>Primero</a></li>                           
                    <?php
                    $inicio_paginador = 1;
                    $fin_paginador = 15;
                    ...
                    if ($total_cursos > 1) {
                        for ($i = $inicio_paginador; $i <= $fin_paginador; $i++) {
                            if ($vista == $i) {
                                echo '<li class="active"><a ' . loadpage('login-log/' . $i) . '>' . $i . '</a></li>';
                            } else {
                                echo '<li><a ' . loadpage('login-log/' . $i) . '>' . $i . '</a></li>';
                            }
                        }
                    }
                    ?>                            
                    <li><a <?php echo loadpage('login-log/' . $total_cursos); ?>>Ultimo</a></li>
                </ul>
            </div><!-- /col-md-12 -->	
        </div>

...
if(isset_post('editar-participante')){
    ...
    $ids_pr_registros = post('ids_pr_registros');
    
    query("UPDATE cursos_participantes SET nombres='$nombres',apellidos='$apellidos',correo='$correo',celular='$celular' WHERE id IN ($ids_participantes) AND id_proceso_registro IN ($ids_pr_registros) ORDER BY id DESC limit 20 ");
    query("UPDATE cursos_proceso_registro SET correo_contacto='$correo',celular_contacto='$celular' WHERE id IN ($ids_pr_registros) AND id_grupo='$id_grupo' ORDER BY id DESC limit 10 ");

    logcursos('Edicion de participante desde grupo ['.$ids_participantes.']', 'grupo-edicion', 'grupo', $id_grupo);
}
...
$resultado1 = query("SELECT * FROM cursos_participantes WHERE id_curso IN (select id_curso from cursos_agrupaciones where id='$id_grupo') AND estado='1' $qr_busqueda ORDER BY id DESC ");

/* datos del grupo */
$rqc1 = query("SELECT * FROM cursos_agrupaciones WHERE id='$id_grupo' ORDER BY id DESC limit 1 ");
$rqc2 = mysql_fetch_array($rqc1);
$nombre_del_curso = $rqc2['titulo'];
$fecha_del_curso = 'NO DATA';
$id_certificado_curso = 0;
$id_certificado_2_curso = 0;
...
<div class="row">
    <div class="col-mod-12">
        ...
        <div class="form-group hiddn-minibar pull-right">
            <div class="hidden-sm">
                <a href="cursos-grupos.adm" class="btn btn-sm btn-info active"><i class="fa fa-group"></i> GRUPOS</a>
                &nbsp;
            </div>
            <div class="hidden-md">
                <a href="cursos-grupos.adm" class="btn btn-sm btn-info active" title="GRUPOS"><i class="fa fa-group"></i></a>
            </div>
        </div>
        <h4 class="page-header"> PARTICIPANTES DE LOS CURSO DEL GRUPO <i class="fa fa-info-circle animated bounceInDown show-info"></i> </h4>
...
<div class="row">
    <div class="col-md-12" style="padding-left: 0px;">
        <h4 style="margin: 0px;">
            <i class='btn btn-success active'>GRUPO <?php echo $id_grupo; ?></i> | 
            <?php echo $nombre_del_curso; ?>

            <span class="pull-right">
                <a class="btn btn-sm btn-primary" <?php echo loadpage('cursos-grupos-participantes/'.$id_grupo); ?>>TODOS</a>
                &nbsp;
                <a class="btn btn-sm btn-primary" <?php echo loadpage('cursos-grupos-participantes/'.$id_grupo.'/conpago'); ?>>CON PAGO</a>
                &nbsp;
                <a class="btn btn-sm btn-primary" <?php echo loadpage('cursos-grupos-participantes/'.$id_grupo.'/sinpago'); ?>>SIN PAGO</a>
                &nbsp;
                <a class="btn btn-sm btn-primary" <?php echo loadpage('cursos-grupos-participantes/'.$id_grupo.'/eliminados'); ?>>ELIMINADOS</a>
            </span>
        </h4>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12NOT">
        <div class="panelNOT">
            ...
            <div class="row" style="background: #e3efd5;padding: 3px 0px;margin: 5px 0px;border-radius: 5px;border: 1px solid #9cbf73;">
                <div class="col-md-12">
                    <form action="" method="post" onsubmit="return busca_participantes(<?php echo $id_grupo; ?>, 0, document.getElementById('inputbuscador').value);">
                        <div class="input-group col-sm-12">
                            <span class="input-group-addon" onclick="busca_participantes(<?php echo $id_grupo; ?>, 0, document.getElementById('inputbuscador').value);" style="cursor:pointer;"><i class="fa fa-search"></i> &nbsp; Buscar: </span>
            ...
            <div class="panel-bodyNOT" style="padding-top: 0px;">
                ...
                <div id="ajaxloading-lista_participantes_grupo"></div>
                <div id="ajaxbox-lista_participantes_grupo">
                    ....
                </div>
        ...
        if (confirm("Desea habilitar nuevamente al participante?")) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //location.href = 'cursos-grupos-participantes/<?php echo $id_grupo; ?>.adm';
                    lista_participantes_grupo('<?php echo $id_grupo; ?>', '<?php echo $filtro; ?>');
                    $('.alert').hide();
                }
            });
...
<script>
    function imprimir_certificados_multiple(dat) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.imprimir_certificados_multiple.php',
            data: {id_grupo: <?php echo $id_grupo; ?>, nro_certificado: dat, ids: ids.join(',')},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function imprimir_certificado_individual(dat) {
        ...
        if (dat > 0) {
            $.ajax({
                url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.imprimir_certificado_individual.php',
                data: {dat: dat},
                type: 'POST',
                dataType: 'html',
                success: function (data) {
...
<script>
    function imprimir_dos_certificados(dat) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.imprimir_dos_certificados.php',
            data: {id_grupo: <?php echo $id_grupo; ?>, ids: dat},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                window.open(data, 'popup', 'width=700,height=500');
...
<script>
    function imprimir_tres_certificados(dat) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.imprimir_tres_certificados.php',
            data: {id_grupo: <?php echo $id_grupo; ?>, ids: dat},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function imprimir_copia_legalizada(dat) {
        ...
        if (dat > 0) {
            $.ajax({
                url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.imprimir_copia_legalizada.php',
                data: {dat: dat},
                type: 'POST',
                dataType: 'html',
                success: function (data) {
                    ...
                    /*
                     setTimeout(function() {
                     lista_participantes_grupo(<?php //echo $id_grupo;                     ?>, 0);
                     }, 2000);
                     */
                }
            });
...
<script>
    function visualizar_certificado_digital(dat) {
        if (dat > 0) {
            $.ajax({
                url: 'contenido/paginas.admin/ajax/ajax.aux.cursos-grupos-participantes.visualizar_certificado_digital.php',
                data: {dat: dat},
                type: 'POST',
                dataType: 'html',
                success: function (data) {
...
<script>
    function emision_multiple_certificados() {
        ...
        $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_certificados.php',
            data: {dat: ids.join(',')},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emision_multiple_c2_certificados() {
        ...
        if (true) {
            ...
            $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
            $.ajax({
                url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_c2_certificados.php',
...
<script>
    function emision_certificados_a_eleccion() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_certificados_a_eleccion.php',
            data: {dat: ids.join(',')},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emision_certificados_a_eleccion_p2() {
        ...
        $("#box-modal_emision_certificados-a-eleccion").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_certificados_a_eleccion_p2.php',
...
<script>
    function emision_certificados_a_eleccion_p3() {
        ...
        $("#box-modal_emision_certificados-a-eleccion").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_certificados_a_eleccion_p3.php',
            data: {dat: ids.join(','), id_certificado: id_certificado, id_grupo: id_grupo, id_modelo_certificado: id_modelo_certificado},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                $("#box-modal_emision_certificados-a-eleccion").html(data);
...
<script>
    function imprime_certificados_a_eleccion() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.impresion_certificados_a_eleccion.php',
            data: {dat: ids.join(',')},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function imprime_certificados_a_eleccion_p2() {
        ...
        $("#box-modal_impresion_certificados-a-eleccion").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.impresion_certificados_a_eleccion_p2.php',
            data: {dat: ids.join(','), id_modelo_certificado: id_modelo_certificado},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emite_certificados_multiple(id_certificado, id_grupo, modo) {
        var ids;
        ids = $('input[type=checkbox]:checked').map(function () {
            return $(this).attr('id');
        }).get();
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.emite_certificados_multiple.php',
            data: {dat: ids.join(','), id_certificado: id_certificado, id_grupo: id_grupo, modo: modo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                $("#AJAXCONTENT-emite_certificados_multiple").html(data);
...
<script>
    function emision_multiple_certificados_p2() {
        ...
        $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_certificados_p2.php',
            data: {dat: ids.join(','), id_certificado: id_certificado, id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                $("#AJAXCONTENT-emite_certificados_multiple").html(data);
...
<script>
    function emision_multiple_c2_certificados_p2() {
        ...
        $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_c2_certificados_p2.php',
            data: {dat: ids.join(','), id_certificado: id_certificado, id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function imprimir_facturas() {
        ...
        window.open('<?php echo $dominio; ?>contenido/librerias/fpdf/tutorial/factura-1-masivo.php?id_participantes=' + ids.join(','), 'popup', 'width=700,height=500');
...
<script>
    function emision_multiple_facturas() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_facturas.php',
            data: {dat: ids.join(',')},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emision_multiple_facturas_p2() {
        ...
        $("#box-modal_emision_facturas-multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_facturas_p2.php',
            data: {dat: ids.join(','), id_certificado: id_certificado, id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function procesar_certificados_secundarios(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.procesar_certificados_secundarios.php',
            data: {id_participante: id_participante, id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function procesar_certificados_secundarios_p2() {
        ...
        $("#BOX-AJAX-certificados-secundarios").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.procesar_certificados_secundarios_p2.php',
            data: arraydata,
            type: 'POST',
            success: function (data) {
                $("#BOX-AJAX-certificados-secundarios").html(data);
...
<script>
    function deshabilita_participantes_no_seleccionados() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.deshabilita_participantes_no_seleccionados_p1.php',
...
<script>
    function deshabilita_participantes_no_seleccionados_p2() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.deshabilita_participantes_no_seleccionados_p2.php',
            data: {dat: aux_idsalmacenador_2},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emision_cupones_infosicoes() {
        ...
        $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_cupones_infosicoes.php',
            data: {dat: ids.join(','), id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function emision_cupones_infosicoes_p2() {
        ...
        $("#AJAXCONTENT-emite_certificados_multiple").html('<img src="contenido/imagenes/images/load_ajax.gif"/>');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.modal.cursos-grupos-participantes.emision_multiple_cupones_infosicoes_p2.php',
            data: {dat: ids.join(','), id_grupo: id_grupo},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function edita_certificado_individual(id_emision_certificado) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.edita_certificado_individual.php',
...
<script>
    function proceso_envio_de_certificado(id_emision_certificado) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.proceso_envio_de_certificado.php',
            data: {id_emision_certificado: id_emision_certificado},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function enviar_factura(id) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.instant.enviar_factura.php?id_factura=' + id + '&email_a_enviar=' + email,
            data: {id: id},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function lista_participantes_grupo(id_grupo, filtro) {
        $("#ajaxloading-lista_participantes_grupo").html(text__loading_tres);
        document.getElementById('inputbuscador').value = "";
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.lista_participantes_grupo.php',
            data: {id_grupo: id_grupo, filtro: filtro, modo_pago: VAR_modo_de_pago},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                $("#ajaxloading-lista_participantes_grupo").html("");
                $("#ajaxbox-lista_participantes_grupo").html(data);
    ...
    function lista_participantes_grupo_INICIO(id_grupo, filtro) {
        $("#ajaxloading-lista_participantes_grupo").html(text__loading_uno);
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.lista_participantes_grupo.php',
            data: {id_grupo: id_grupo, filtro: filtro, modo_pago: VAR_modo_de_pago},
            type: 'POST',
    ...
    function busca_participantes(id_grupo, id_turno, busc) {
        $("#ajaxloading-lista_participantes_grupo").html(text__loading_tres);
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.lista_participantes_grupo.php',
            data: {id_grupo: id_grupo, id_turno: id_turno, busc: busc, modo_pago: VAR_modo_de_pago},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
                $("#ajaxloading-lista_participantes_grupo").html("");
    ...
    function edita_participante_p1(id_participante, nro_lista) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.edita_participante_p1.php',
    ...
    /*
     ...
     $.ajax({
     url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.edita_participante_p2.php',
    ...
    function emite_certificado_p1(id_participante, nro_certificado) {
        ...
        $("#ajaxbox-emite_certificado_p1").html("");
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.emite_certificado_p1.php',
    ...
    function emite_certificado_p2(id_participante, nro_certificado) {
        ...
        var id_grupo = $("#id_grupo-" + id_participante).val();
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.emite_certificado_p2.php',
            data: {receptor_de_certificado: receptor_de_certificado, id_certificado: id_certificado, id_grupo: id_grupo, id_participante: id_participante, nro_certificado: nro_certificado, cont_tres: cont_tres, fecha_qr: fecha_qr, cont_dos: cont_dos, texto_qr: texto_qr},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function datos_registro(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.datos_registro.php',
    ...
    function emite_factura_p1(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.emite_factura_p1.php',
            data: {id_participante: id_participante},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function emite_factura_p2(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.emite_factura_p2.php',
            data: data_form,
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function elimina_participante_p1(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.elimina_participante_p1.php',
            data: {id_participante: id_participante},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function elimina_participante_p2(id_participante, apartar) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.elimina_participante_p2.php',
    ...
    function habilita_participante_p1(id_participante, apartar) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.habilita_participante_p1.php',
            data: {id_participante: id_participante, apartar: apartar},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function habilita_participante_p2(id_participante, apartar) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.habilita_participante_p2.php',
            data: {id_participante: id_participante, apartar: apartar},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function agrega_participante() {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.agrega_participante.php',
            data: data_form,
            type: 'POST',
            dataType: 'html',
            success: function (data) {
    ...
    function agregar_participantes_csv() {
        ...
        $.ajax({
            url: "contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.agregar_participantes_csv.php",
            data: fd,
            cache: false,
            processData: false,
            contentType: false,
...
<script>
    function cambiar_estado_curso(id_grupo, estado) {
        $("#box-desactivar-curso").html("Cargando...");
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.cambiar_estado_curso.php',
...
<script>
    ...
    lista_participantes_grupo_INICIO('<?php echo $id_grupo; ?>', '<?php echo $filtro; ?>');
...
<script>
    function checkParticipante(dat) {
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.checkParticipante.php',
            data: {dat: dat},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function historial_participante(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.historial_participante.php',
            data: {id_participante: id_participante},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function reprogramacion_de_curso(id_participante) {
        ...
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.reprogramacion_de_curso.php',
            data: {id_participante: id_participante},
            type: 'POST',
            dataType: 'html',
            success: function (data) {
...
<script>
    function reprogramacion_de_curso_p2() {
        ...
        $("#TR-AJAXBOX-reprogramacion_de_curso").html('Cargando...');
        $.ajax({
            url: 'contenido/paginas.admin/ajax/ajax.cursos-grupos-participantes.reprogramacion_de_curso_p2.php',
            data: form,
            type: 'POST',
            dataType: 'html',
            success: function (data) {

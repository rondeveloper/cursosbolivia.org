    ...
    <div class="col-md-5">
        <div class="panel panel-warning">
            ...
            <div class="panel-body">
                <table class="table table-bordered table-striped">
                    <?php
                    $rqdc1 = query("SELECT id,titulo,fecha,(select nombre from ciudades where id=cursos.id_ciudad)ciudad FROM cursos WHERE estado IN (1,2) AND id IN (select id_curso from cursos_rel_cursowapnum where id_whats_numero='$id_whats_numero') ORDER BY id_ciudad ASC, fecha ASC limit 100 ");
                    $cur_ciudad= '';
                    while ($rqdc2 = mysql_fetch_array($rqdc1)) {
                        ...
                        ?>
                        <tr>
                            <td><?php echo date("d/M", strtotime($rqdc2['fecha'])); ?></td>
        ...
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                <table class="table table-bordered table-striped table-hover">
                    <?php
                    $rqdc1 = query("SELECT id,titulo,fecha,(select nombre from ciudades where id=cursos.id_ciudad)ciudad FROM cursos WHERE estado IN (1,2) AND id NOT IN (select id_curso from cursos_rel_cursowapnum where id_whats_numero='$id_whats_numero') ORDER BY id_ciudad ASC, fecha ASC limit 100 ");
                    $cur_ciudad= '';
                    while ($rqdc2 = mysql_fetch_array($rqdc1)) {
                        if($rqdc2['ciudad'] != $cur_ciudad){
                            $cur_ciudad = $rqdc2['ciudad'];

...
/* current data curso */
$rqdcc1 = query("SELECT fecha,numero FROM cursos_agrupaciones WHERE id='$id_grupo' ORDER BY id DESC limit 1 ");
...
if ((strtotime($current_fecha) == strtotime(date("Y-m-d"))) && (strtotime($fecha) > strtotime(date("Y-m-d"))) && false) {
    ...
    if (isset_post('sw_accept_cdate') && post('sw_accept_cdate') == '1') {
        ...
        /* tags */
        $rqdcct1 = query("SELECT id_tag FROM cursos_rel_cursostags WHERE id_curso='$id_grupo' ");
        while ($rqdcct2 = mysql_fetch_array($rqdcct1)) {
            $id_tag = $rqdcct2['id_tag'];
            $rqverif1 = query("SELECT COUNT(1) AS total FROM cursos_rel_cursostags WHERE id_curso='$id_grupo_nuevo' AND id_tag='$id_tag' ");
            $rqverif2 = mysql_fetch_array($rqverif1);
            if ($rqverif2['total'] == 0) {
                query("INSERT INTO cursos_rel_cursostags (id_curso,id_tag) VALUES ('$id_grupo_nuevo','$id_tag') ");
            }
        }
        logcursos('Creacion de curso por duplicacion[' . $id_grupo . ':' . $titulo_curso . ']', 'curso-creacion', 'curso', $id_grupo_nuevo);
        ...
        logcursos('Edicion de datos de curso [TITULO/FECHA/COSTOS]', 'curso-edicion', 'curso', $id_grupo);
        ?>
...
} elseif ((strtotime($fecha) < strtotime(date("Y-m-d"))) && false) {
    /* fecha pasada */
    ?>
    ...
    <?php
} elseif (strtotime($fecha) >= strtotime(date("Y-m-d")) || true) {
    /* fecha posterior */
    ...
    logcursos('Edicion de datos de grupo [TITULO/FECHA/COSTOS]', 'grupo-edicion', 'grupo', $id_grupo);
    ?>

...
switch ($estado) {
    case 'activado':
        ...
        logcursos('Cambio de estado [ACTIVADO]', 'curso-edicion', 'grupo', $id_grupo);
        break;
    case 'desactivado':
        ...
        logcursos('Cambio de estado [DES-ACTIVADO]', 'curso-edicion', 'grupo', $id_grupo);
        break;
    case 'temporal':
        $titulo_identificador .= '-tmp';
        query("UPDATE cursos_agrupaciones SET estado='2',titulo_identificador='$titulo_identificador',laststch_fecha='$laststch_fecha',laststch_id_administrador='$laststch_id_administrador' WHERE id='$id_grupo' ORDER BY id DESC limit 1 ");
        logcursos('Cambio de estado [TEMPORAL]', 'curso-edicion', 'grupo', $id_grupo);
        break;
    default :
        break;
}
...
/* curso */
$rqdc1 = query("SELECT id,estado,columna FROM cursos_agrupaciones WHERE id='$id_grupo' ORDER BY id DESC limit 1 ");
...
if ($producto['estado'] == '1') {
    ...
    if (acceso_cod('adm-cursos-estado')) {
        ...
        <div class="col-md-2">
            <div>
                <input type="button" value="TERMPORALIZAR" class="btn btn-danger btn-sm btn-block" onclick="cambiar_estado_grupo('<?php echo $producto['id']; ?>', 'temporal');"/>
            </div>
        </div>
        <div class="col-md-2">
            <div>
                <input type="button" value="DESACTIVAR" class="btn btn-default btn-sm btn-block" onclick="cambiar_estado_grupo('<?php echo $producto['id']; ?>', 'desactivado');"/>
            </div>
        </div>
        <?php
    }
    ...
    if (acceso_cod('adm-cursos-estado')) {
        ...
        <div class="col-md-2">
            <div>
                <input type="button" value="ACTIVAR" class="btn btn-success btn-sm btn-block" onclick="cambiar_estado_grupo('<?php echo $producto['id']; ?>', 'activado');"/>
            </div>
        </div>
        <div class="col-md-2">
            <div>

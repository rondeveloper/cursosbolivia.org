...
/* estado actual */
$fp = fopen("../../configuracion/sw_ghtmls.dat", "r");
...
function genera_index() {
    ...
    $html = file_get_contents("https://cursos.bo");
    ...
    if (false) {
        ...
        while ($rqc2 = mysql_fetch_array($rqc1)) {
            ...
            if (file_exists($ruta_final_archivo)) {
                ...
                $html = file_get_contents("https://cursos.bo/" . $titulo_identificador . ".html");
                $compress = TinyMinify::html($html);
                /* crea archivo fisico */
                $archivo = fopen($ruta_final_archivo, "w");
                fwrite($archivo, $compress);
...
function elimina_htmls_cursos_noactivos() {
    ...
    while ($archivo = readdir($directorio)) {
        ...
        $ruta_final_archivo = $ruta_htmls . $archivo;
        if (!is_dir($archivo) && ($archivo !== $prefijo_de_archivo . 'index' . $formato) && mysql_num_rows($rqveca1) == 0 && file_exists($ruta_final_archivo)) {
            unlink($ruta_final_archivo);
            $cnt++;
        }
    }

...
if(isset_get('ids_emisiones')){
    ...
    $rqce1 = query("SELECT certificado_id FROM cursos_emisiones_certificados WHERE id IN (select id_emision_certificado from cursos_participantes where id in($id_participantes) ) OR id IN (select id_emision_certificado_2 from cursos_participantes where id in($id_participantes) ) OR id IN (select id_emision_certificado_3 from cursos_participantes where id in($id_participantes) ) ORDER BY id_participante ASC limit 600 ");
...
while ($rqce2 = mysql_fetch_array($rqce1)) {
    ...
    $participante = mysql_fetch_array($rqdpc1);
    ...
    $cont_tres = utf8_decode(str_replace('[DEPARTAMENTO-PARTICIPANTE]','BOLIVIA',$rqc2['cont_tres']));
...
if ($ci_participante !== '' && $ci_participante !== '0') {
    ...
    /* numeracion */
    if(isset_get('enconjuntoalfabetico') && get('enconjuntoalfabetico')=='true'){
        /* $codnum_c SIMULADO */
        $codnum_c = '0';
        $rqcesc1 = query("SELECT nombres,apellidos FROM cursos_participantes WHERE id_curso='$id_curso' AND estado='1' ORDER BY id ASC ");
        $cnt_counter_aux_counter = 0;
        while ($rqcesc2 = mysql_fetch_array($rqcesc1)) {

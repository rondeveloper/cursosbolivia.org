...
/* limite */
$qr_limit = '';
if (isset($dir[2]) && ((int) $dir[2] > 0)) {
    $qr_limit = ' LIMIT ' . (int) $dir[2] . ' ';
}
?>
<div class="gris">
    <section class="container">
        ...
        if (num_rows($rc1) == 0) {
            ...
            //echo "<p>No hay cursos disponibles en este departamento por el momento.</p><br/>";
            echo "</div>";
            echo "</div>";
            echo "</div>";
        }
    ...
    <section class="container">
        <blockquote>
            <p>A continuaci&oacute;n te mostramos informaci&oacute;n de las entidades financieras abiertas en el periodo de cuarentena en este departamento.</p>
        </blockquote>
        <?php
        $rqdef1 = query("SELECT * FROM departamentos WHERE estado='1' AND id='$id_departamento' ORDER BY orden ASC ");
        $rqdef2 = fetch($rqdef1);
        ?>
    ...
    if (false) {
        ...
        <section class="container">
            <div class="row">
                <?php
                $rc1 = query("SELECT c.id,c.titulo,c.titulo_identificador,c.imagen,c.fecha FROM cursos c WHERE estado='0' AND id_ciudad IN (select id from ciudades where id_departamento='$id_departamento') AND sw_flag_cursosbo='1' ORDER BY fecha ASC limit 10 ");
                if (num_rows($rc1) == 0) {

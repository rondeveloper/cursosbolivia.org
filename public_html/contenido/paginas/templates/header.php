...
/* certificados con costo enlace directo */
if (isset($dir[1]) && ($dir[1]=='certificados')) {
    $get[1] = 'aut-certificados';
    $get[2] = $dir[2];
}
...
if (isset($get[1])) {
    ...
    /* matatag curso / registro-curso */
    if ($get[1] == 'curso' || $get[1] == 'registro-curso') {
        $rqc1 = query("SELECT titulo,imagen,contenido,costo FROM cursos WHERE titulo_identificador='" . $get[2] . "' AND estado IN (0,1,2,3) ORDER BY FIELD(estado,1,2,0,3),id DESC limit 1 ");
        if (num_rows($rqc1) > 0) {
            $rqc2 = fetch($rqc1);
            $titulo_pagina = $rqc2['titulo'];
            $costo_curso = $rqc2['costo'];
            if($get[1] == 'registro-curso' && $costo_curso != 0){
                $titulo_pagina = '𝙍𝙚𝙥𝙤𝙧𝙩𝙖𝙧 𝙥𝙖𝙜𝙤 >> '.$titulo_pagina;
            }
            $descripcion_pagina = str_replace('[NOMBRE-CURSO]',$rqc2['titulo'],substr(strip_tags($rqc2['contenido']), 0, 300)) . '...';
            $image_pagina = $dominio . 'contenido/imagenes/paginas/' . str_ireplace(' ','%20',$rqc2['imagen']);

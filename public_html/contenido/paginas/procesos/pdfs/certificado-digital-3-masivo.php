...
if(isset_get('ids_emisiones')){
    $rqcev1 = query("SELECT certificado_id FROM cursos_emisiones_certificados WHERE id IN (".get('ids_emisiones').") ORDER BY id_participante ASC limit 600 ");
}

$cnt_counter = num_rows($rqcev1);
if ($cnt_counter == 0) {
...
$nombre_archivo = strtolower($nom_file_participante.'-'.date("HidmY")."-CERTIFICADOS.pdf");
/* output */

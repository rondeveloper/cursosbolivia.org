...
while ($rqce2 = fetch($rqce1)) {
    ...
    $cont_uno = utf8_decode('duración del paquete: ') . $duracion . ' MESES';
    $cont_dos = utf8_decode('CÓDIGO DE CUPÓN: ' . $codigo);
    $cont_tres = utf8_decode('Este cupón tiene validez hasta: ' ). date("d / m / Y", strtotime($fecha_expiracion_cupon));
...
//$pdf->Cell(0, 30, 'Profesionales, Técnicos y Consultores", con una carga horaria de 8 horas.', 0, 0, 'C');
    ...
    $pdf->SetY(190);
    $pdf->SetX(25);
    $pdf->MultiCell(550, 14, utf8_decode('Este cupón fue emitido para '.$nombre_participante.' y constituye la adquisición de un paquete de servicios de la plataforma Infosiscon de monitoreo de licitaciones de Bolivia.'), 0, 'L');
...
if(isset_get('download') && get('download')=='true'){
    $modo = "D";
    $nombre_archivo = "CUPON-$codigo.pdf";

...
$qr_filtro_busqueda = '';
if(isset_post('input-buscar')){
    $key_search = str_replace(' ','%',post('input-buscar'));
    $qr_filtro_busqueda = " AND c.titulo LIKE '%$key_search%' ";
}
?>
<div class="row">
    <?php
    ...
    if (false) {
    ?>
        <div class="cont-short-couses" id="shcc-0">
        <?php
    }
        ?>
        <?php
        ...
        while ($rc2 = fetch($rcv1)) {
            ...
            $rqdwn1 = query("SELECT w.numero ,w.wap_shortlink FROM whatsapp_numeros w INNER JOIN cursos_rel_cursowapnum r ON r.id_whats_numero=w.id WHERE r.id_curso='" . $rc2['dr_id_curso'] . "' ORDER BY r.id ASC LIMIT 1 ");
            $num_whatsapp = "69714008";
            $wap_shortlink = 'https://wa.me/59169714008';
            if (num_rows($rqdwn1)>0) {
                $rqdwn2 = fetch($rqdwn1);
                $num_whatsapp = $rqdwn2['numero'];
                if($rqdwn2['wap_shortlink']!=''){
                    $wap_shortlink = $rqdwn2['wap_shortlink'];
                }
            }
        ...
        ?>
            <div class="col-xs-12 col-sm-6 col-md-4" align="left">
                <div class="blog-post-short">
                    ...
                    <?php }else{ ?>
                        <div class="box-label-costo_no_mostrar">&nbsp;</div>
                    <?php } ?>
                </div>
                <!----->
                <div class="blog-post-short list-group-item ">
                    ...
                    <div class="row hi  ">
                        <div class="blog-meta">
                            <div class="col-md-4 col-xs-4 hi " align="right">
                            ...
                            <div class="col-md-2 col-xs-2 hi " style="padding: 5px;text-align: center;">
                                <b class="btn btn-xs btn-info" onclick="copyToClipboard('R-<?php echo $rc2['dr_id_curso']; ?>');" title="Copiar informacion al clipboard." style="width: 100%;border-radius: 3px;"><i class="icon-edit text-contrast"></i></b>
                            </div>
                            <div class="col-md-4 col-xs-4 hi " align="right">
            ...
            <div id="contentInfo-R-<?php echo $rc2['dr_id_curso']; ?>" style="display:none;">https://cursos.bo/R/<?php echo $rc2['dr_id_curso']; ?>/</div>
        ...
        <div class="cover-short-courses" onclick="$('#shcc-0').removeClass('cont-short-couses');$('#shcc-labelshowmore-0').css('display', 'none');" id="shcc-labelshowmore-0">
            <b class="cover-short-courses-label">...Mostrar todo...</b>
        </div>
    <?php
        }
    ?>

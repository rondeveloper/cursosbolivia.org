...
function editorTinyMCE($id_elemento) {
    global $dominio_www;
    ?><script src="<?php echo $dominio_www; ?>contenido/tinymce/js/tinymce/tinymce.min.js"></script>
    <script type="text/javascript">
        tinymce.init({
            language: 'es',
            theme: "modern",
...
function getContenidoCurso($curso) {
    ...
    $rqdtcb1 = query("SELECT c.*,(b.nombre)nombre_banco FROM rel_cursocuentabancaria r INNER JOIN cuentas_de_banco c ON r.id_cuenta=c.id INNER JOIN bancos b ON c.id_banco=b.id WHERE r.id_curso='".$curso['id']."' AND r.sw_cprin=1 AND r.estado=1 ORDER BY c.id ASC ");
...
function platillaEmailUno($bodyEmail,$tituloEmail,$enviarAEmail,$urlUnsubscribeEmail,$nomUsuarioEmail) {
    global $dominio,$___nombre_del_sitio,$___color_base;
...
<img alt="" style="display:block;height:auto!important;max-width:100%!important;" width="599" vspace="0" hspace="0" border="0" src="'.$dominio.'contenido/alt/logotipo-v3.png">
</div>
...
function SISTsendEmail_PRE($correo_a_enviar, $asunto, $contenido_correo) {
...
function SISTsendEmailFULL($correo_a_enviar, $asunto, $contenido_correo,$copias,$archivos) {
    if (emailValido($correo_a_enviar)) {
        $id_correo_notificador = 1;
        /*
        $arc1 = explode('@',$correo_a_enviar);

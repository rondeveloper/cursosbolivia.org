...
$rqcp1 = query("SELECT p.*,r.id_emision_certificado FROM cursos_participantes p INNER JOIN cursos_rel_partcertadicional r ON p.id=r.id_participante WHERE p.estado='1' AND p.id IN ($ids_participantes) AND r.id_certificado='$id_certificado' ORDER BY p.id DESC ");
...
<!--                        <td>
            ...
            <center id="panel-1">
                <b class="btn btn-success" onclick="envia_certificados_multiple_individualmente();">ENVIAR CERTIFICADOS</b>
                &nbsp;&nbsp;&nbsp;
...
<!-- AJAX envia_certificados_multiple_individualmente -->
<script>
    var sw_envios = true;
    function envia_certificados_multiple_individualmente() {
    ...
    function enviaIndividual(arr_ids) {
        if(arr_ids.length>0 && sw_envios){
            ...
            if(item!=='0'){
                ...
                $.ajax({
                        url: '<?php echo $dominio_procesamiento; ?>admin/process.cursos-participantes.enviar_certificado.php',

...
$data_report_departamento = '0';
if (isset_post('data_report_departamento')) {
    $data_report_departamento = '1';
    $cnt_campos++;
}
...
$data_report_celular = '0';
if (isset_post('data_report_celular')) {
    $data_report_celular = '1';
    $cnt_campos++;
}
$data_report_correo = '0';
if (isset_post('data_report_correo')) {
    $data_report_correo = '1';
    $cnt_campos++;
}
...
$data_incluir_conpago = '0';
if (isset_post('data_incluir_conpago')) {
    $data_incluir_conpago = '1';
    $cnt_campos++;
}
$data_incluir_sinpago = '0';
if (isset_post('data_incluir_sinpago')) {
    $data_incluir_sinpago = '1';
    $cnt_campos++;
}
$data_ci = '0';
...
$data_incluir_notas = '0';
if (isset_post('data_incluir_notas')) {
    $data_incluir_notas = '1';
    $cnt_campos++;
}


$data_required = 'data_report_nombres=' . $data_report_nombres . '&data_report_apellidos=' . $data_report_apellidos . '&data_report_celular='.$data_report_celular.'&data_report_correo='.$data_report_correo.'&data_report_datosfacturacion=' . $data_report_datosfacturacion . '&data_report_datoscontacto=' . $data_report_datoscontacto . '&data_report_firma=' . $data_report_firma . '&data_report_prefijo=' . $data_report_prefijo . '&data_report_departamento='.$data_report_departamento.'&data_report_fecharegistro=' . $data_report_fecharegistro . '&data_report_modoregistro=' . $data_report_modoregistro . '&data_report_eliminados=' . $data_report_eliminados . '&data_numeracion_certificado=' . $data_numeracion_certificado . '&data_incluir_sinpago='.$data_incluir_sinpago.'&data_incluir_conpago='.$data_incluir_conpago.'&data_report_montopago=' . $data_report_montopago . '&data_report_regpago=' . $data_report_regpago . '&data_id_turno=' . $data_id_turno . '&data_ci=' . $data_ci. '&data_solo_facturados=' . $data_solo_facturados. '&data_incluir_notas=' . $data_incluir_notas;
...
if ($proceso == 'cierre') {
    ...
    $url_content = $dominio_admin.'pages/ajax/ajax.impresion.cursos-participantes.exportar-lista.php?id_curso=' . $id_curso . '&html=true&' . $data_required . '&k3y74ue8=true&k3y74ue8_numeracion=' . $numeracion . '&k3y74ue8_cod_documento=' . $cod_documento . '&k3y74ue8_apartado=' . $apartado;
...
// instantiate and use the dompdf class
    $dompdf = new Dompdf();
    $dompdf->loadHtml((str_replace($busc_imagenes, $remm_imagenes, $varrr)));
...
// Output the generated PDF to Browser
    ...
    file_put_contents("../../../contenido/archivos/documentos/" . $nombre_archivo, $output);
    ...
    $url_documento = $dominio_www."contenido/archivos/documentos/$nombre_archivo";
    ?>
    ...
    <script>
        ...
        function generar_s_reporte(formato) {
            window.open('<?php echo $dominio_admin; ?>pages/ajax/ajax.impresion.cursos-participantes.exportar-lista.php?id_curso=<?php echo $id_curso; ?>&' + formato + '=true&' + '<?php echo $data_required; ?>' + '&rimp_cod_documento=<?php echo $cod_documento; ?>', 'popup', 'width=700,height=500');
        }
    </script>
    <?php
} else {
    ...
    <script>
        ...
        function generar_s_reporte(formato) {
            window.open('<?php echo $dominio_admin; ?>pages/ajax/ajax.impresion.cursos-participantes.exportar-lista.php?id_curso=<?php echo $id_curso; ?>&' + formato + '=true&' + '<?php echo $data_required; ?>', 'popup', 'width=700,height=500');
        }
    </script>
    <?php
}

...
if($mod=='3'){
    ...
    $aux_txt_depo = "-fotografias-";
    $aux_dir_depo = " FOTOGRAFIAS";
}
...
$rqdl1 = query("SELECT u.id,u.nombres,u.apellidos FROM cursos_participantes p INNER JOIN cursos_usuarios u ON p.id_usuario=u.id INNER JOIN documentos_usuario d ON d.id_usuario=u.id WHERE p.id_curso='$id_curso' AND p.id IN ($ids) $qr_depo $qr_foto GROUP BY u.id ORDER BY p.apellidos ASC ");
if(num_rows($rqdl1)==0){
...
while ($rqdl2 = fetch($rqdl1)) {
    ...
    while ($rqddu2 = fetch($rqddu1)) {
        ...
        if($mod=='3'){
            $num++;
            $dest = $micarpeta_part.'/'.$num.'-'.strtoupper(limpiar_enlace($rqdl2['apellidos'] . ' ' . $rqdl2['nombres'])).'__'.substr($rqddu2['nombre'],strlen($rqddu2['nombre'])-5);
        }else{

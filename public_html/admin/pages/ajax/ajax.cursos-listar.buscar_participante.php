            ...
            while ($rqdp2 = fetch($rqdp1)) {
                ...
                <td style="line-height: 2;">
                    ...
                    <td style="line-height: 2;">
                        ...
                        <?php 
                        $htm_motivo_eliminado = '';
                        if($rqdp2['estado']=='1'){
                            echo '<b class="label label-success">Habilitado</b> &nbsp;';
                        }else{
                            echo '<b class="label label-danger">Eliminado</b> &nbsp;';
                            $rqdel1 = query("SELECT e.*,(a.nombre)dr_administrador FROM eliminacion_participantes e INNER JOIN administradores a ON e.id_administrador=a.id WHERE e.id_participante='".$rqdp2['dr_id_participante']."' ORDER BY e.id DESC limit 1 ");
                            if(num_rows($rqdel1)==0){
                                $htm_motivo_eliminado = "<br>Sin motivo de eliminacion registrado";
                            }else{
                                $rqdel2 = fetch($rqdel1);
                                $htm_motivo_eliminado = "<br><b>Motivo eliminaci&oacute;n:</b> ".$rqdel2['motivo']."<br>";
                                $htm_motivo_eliminado .= "<b>Archivo adjunto:</b> ".($rqdel2['archivo']==''?'Sin archivo':"<a href='".$dominio_www."contenido/imagenes/doc-usuarios/".$rqdel2['archivo']."' target='_blank'>Ver archivo</a>")."<br>";
                                $htm_motivo_eliminado .= "<b>Administrador:</b> ".$rqdel2['dr_administrador']."<br>";
                                $htm_motivo_eliminado .= "<b>Fecha:</b> ".date("d/m/Y H:i",strtotime($rqdel2['fecha']))."<br>";
                            }
                        }
                        if($rqccrt2['total']=='0'){

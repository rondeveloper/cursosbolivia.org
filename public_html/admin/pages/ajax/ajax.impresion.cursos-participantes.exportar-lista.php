...
if (isset_administrador() || isset_get('k3y74ue8')) {
    ...
    $sw_data_incluir_conpago = false;
    if (get('data_incluir_conpago') == '1') {
        $sw_data_incluir_conpago = true;
    }
    $sw_data_incluir_sinpago = false;
    if (get('data_incluir_sinpago') == '1') {
        $sw_data_incluir_sinpago = true;
    }
    $sw_data_incluir_notas = false;
    if (get('data_incluir_notas') == '1') {
        $sw_data_incluir_notas = true;
    }
    ...
    $qr_turno = "";
    if ($data_id_turno > 0) {
        $qr_turno = " AND p.id_turno='$data_id_turno' ";
    } else {
    ...
    if($data_solo_facturados){
        ...
        $qr_solofacturados = " AND  r.id_emision_factura>0  ";
    }
    ...
    $qr_pago = "";
    if($sw_data_incluir_sinpago){
        $qr_pago = " AND r.sw_pago_enviado='0' ";
    }
    if($sw_data_incluir_conpago){
        $qr_pago = " AND r.sw_pago_enviado='1' ";
    }
    if($sw_data_incluir_sinpago && $sw_data_incluir_conpago){
        $qr_pago = "";
    }
        ...
        <tr>
            ...
            if ($sw_doc_cierre) {
                ...
                <td style="text-align:center;">
                    <b style='font-size:25pt;'><?php echo $id_curso; ?></b>
                    <br/>
                    ID DE CURSO
                </td>
                <?php
    ...
    <table class="table users-table table-condensed table-hover" style="width:100%;">
        <thead>
            <tr>
                ...
                ?>
                <?php
                if ($sw_data_report_departamento) {
                    ?>
                    <th>Departamento</th>
                    <?php
                }
            ...
            while ($participante = fetch($resultado1)) {
                ...
                <tr id="ajaxbox-tr-participante-<?php echo $participante['id']; ?>">
                    ...
                    if ($sw_data_report_departamento) {
                        ...
                        <td>
                            <?php
                            echo selutf(trim($participante['dr_nombre_departamento']));
                            ?>
                        </td>
                        <?php
                    }
                    ...
                    <?php
                    if ($sw_data_incluir_notas) {
                        $rqntp1 = query("SELECT nota FROM participantes_notas_manuales WHERE id_participante='".$participante['id']."' ORDER BY id  DESC limit 1 ");
                        $nota = 0;
                        if(num_rows($rqntp1)>0){
                            $rqntp2 = fetch($rqntp1);
                            $nota = $rqntp2['nota'];
                        }
                        ?>

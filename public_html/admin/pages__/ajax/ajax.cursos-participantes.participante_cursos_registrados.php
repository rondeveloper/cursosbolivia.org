...
/* data participante */
$rqdcp1 = query("SELECT nombres,apellidos,ci FROM cursos_participantes WHERE id='$id_participante' ORDER BY id DESC limit 1");
$rqdcp2 = fetch($rqdcp1);
$aux_nombres = $rqdcp2['nombres'];
$aux_apellidos = $rqdcp2['apellidos'];
$aux_ci = $rqdcp2['ci'];
...
<div class="alert alert-warning" style="margin:50px;">
  ...
  <br>
  Este listado se hizo en base al nombre: <b><?php echo $aux_nombres.' '.$aux_apellidos; ?></b> y al n&uacute;mero de CI: <b><?php echo $aux_ci; ?></b>.
</div>

<?php

/* data participantes */
$rqddcrp1 = query("SELECT DISTINCT id_curso FROM cursos_participantes WHERE ( nombres LIKE '%$aux_nombres%' AND apellidos LIKE '%$aux_apellidos%' ) OR ci LIKE '$aux_ci' ORDER BY id_curso DESC ");
while($rqddcrp2 = fetch($rqddcrp1)){

    $id_curso = $rqddcrp2['id_curso'];
    ...
    /* busqueda */
    $qr_busqueda = " AND ( ( nombres LIKE '%$aux_nombres%' AND apellidos LIKE '%$aux_apellidos%' ) OR ci LIKE '$aux_ci' ) ";
    /*
    $busqueda = "";
    ...
    /* datos de curso */
    $rqc1 = query("SELECT estado,id_certificado,id_certificado_2,id_certificado_3,id_material,titulo,mailto_subject,mailto_content,sw_askfactura,sw_ipelc,mod_notas FROM cursos c WHERE id='$id_curso' ORDER BY id DESC limit 1 ");
    $rqc2 = fetch($rqc1);
    ...
    if($rqc2['estado']=='1'){
        $estado_curso = 'ACTIVADO';
    }elseif($rqc2['estado']=='2'){
        $estado_curso = 'TEMPORAL';
    }else{
        $estado_curso = 'DESACTIVADO';
    }
    ...
    pr.codigo,pr.fecha_registro,pr.celular_contacto,pr.correo_contacto,pr.id_modo_pago,pr.id_emision_factura,pr.monto_deposito,pr.imagen_deposito,pr.razon_social,pr.nit,pr.cnt_participantes,pr.id_cobro_khipu,pr.sw_pago_enviado,pr.id_administrador,
    ...
    <div style="padding: 20px;border: 2px dashed #c3c3c3;border-radius: 20px;margin-bottom: 20px;">

        <h4 class="titulo-head-panelcurso"><b style="background: #FFF;color: #ff4141;border: 1px solid #ff7869;padding: 2px 10px;border-radius: 5px;"><?php echo $id_curso; ?></b> &nbsp; <?php echo $nombre_curso; ?><i class="pull-right">[<?php echo $estado_curso; ?>]</i></h4>
        <br>
        <style>
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    while ($participante = fetch($resultado1)) {
                        ...
                        if($participante['sw_pago_enviado']=='1' && $participante['dr_id_verifpago']!=""){
        ...
        *TRANSFERENCIA BANCARIA:*
        ...
        ';
                            ...
                            <td class="simple-td text-center">
                                ...
                                ?>
                                <?php 
                                if ($sw_askfactura_curso=='1' && $_SW_pago_verificado) {
                            ...
                            <?php if ($sw_col_act_suev || $sw_col_material || $sw_col_szoom) { ?>
                                <td class="simple-td">
                                    <?php if ($sw_col_szoom && $_SW_pago_verificado) { ?>
                                        <b>Sesion ZOOM:</b>
                                        <div id="td-envszoom-<?php echo $participante['id']; ?>">
                                            <?php
                                            $rqdmev1 = query("SELECT id FROM rel_partszoom WHERE id_participante='".$participante['id']."' ORDER BY id DESC limit 1 ");
                                            if(num_rows($rqdmev1)==0){
                                                ?>
                                                <i style="color: red;">Sin invitaci&oacute;n</i>
                                                <br>
                                                <?php if($sw_habilitacion_procesos){ ?>
                                                <b class="btn btn-xs btn-primary" onclick="enviar_invitacion_individual_zoom('<?php echo $participante['id']; ?>');" style="background: #f36e61;border-color: #dadada;padding: 5px 10px;">Enviar inv. ZOOM</b>
                                                <?php } ?>
                                                <?php
                                            }else{
                                                ?>
                                                <br>
                                                <b class="text-success" style="background: #2b9ae8;color: #FFF;padding: 10px;">Inv. ZOOM enviado</b>
                                                <?php
                                            }
                                            ?>
                                        </div>
                            ...
                            <td class="simple-td">
                                        ...
                                        if ($cnt_cursos_cirtuales_asociados == 1) {
                                            ...
                                            if (num_rows($rqvac1)>0) {
                                                ...
                                                ?>
                                                    ...
                                                    <?php if ($sw_habilitacion_procesos) { ?>
                                                    <div style="padding: 5px;text-align: center;border: 1px solid #EEE;">
                                                        Habilitar: &nbsp; <b class="btn btn-success btn-xs" onclick="habilita_participante_cvirtual_p1(<?php echo $participante['id']; ?>);"><i class="fa fa-check"></i></b>
                                                    </div>
                                                    <?php } ?>
                                ...
                                <br>
                                <?php 
                                if($sw_show_certificados && $_SW_pago_verificado){
                            ...
                            <td class="simple-td">
                                ...
                                if($participante['id_usuario']!='0'){
                                    ?>
                                    <span class="btn btn-xs btn-default btn-block" onclick="usuario_data('<?php echo $participante['id_usuario']; ?>','<?php echo $id_curso; ?>');" data-toggle="modal" data-target="#MODAL-usuario_data" style="color: #2da210;"><i class="fa fa-user"></i> Usuario</span>
                                    <?php
                                }
                                ?>
                                ...
                                <?php
                                if ($sw_habilitacion_procesos && (!$_SW_pago_verificado)) {
                                    ?>
                                    <br>
                                ...
                                <?php } ?>
                                <?php if ($sw_cupon_infosicoes && $_SW_pago_verificado) { ?>
                                    <br>
                                    <br>
...
<!--usuario_data-->
<script>
    function usuario_data(id_usuario,id_curso) {
    ...
    function enviar_invitacion_individual_zoom(id_participante) {
        if(confirm('DESEA ENVIAR LA INVITACION ZOOM ?')){
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log(data);
                    $("#td-envszoom-"+id_participante).html('<br><b class="text-success" style="background: #2b9ae8;color: #FFF;padding: 10px;">Inv. ZOOM enviado</b>');
                }
            });            
        }
    }

...
if (num_rows($rqcp1) == 0) {
    ...
    <div id="AJAXCONTENT-emite_certificados_multiple_p2">
        <form id="FORM-emite_certificados_multiple_p2" action='' method='post'>
            ...
            <center id="panel-1">
                <b class="btn btn-success" onclick="enviar_todos_los_certificados(0);">ENVIAR TODOS LOS CERTIFICADOS EN PDFs</b>
                <br>
                <br>
                <b class="btn btn-success" onclick="enviar_todos_los_certificados(1);">ENVIAR TODOS LOS CERTIFICADOS EN LINKs</b>
                <br>
                <br>
...
<script>
    var sw_envios = true;
    function enviar_todos_los_certificados(solo_links) {
        if(confirm('Desea enviar todos los certificados por correo ? (COMO CERTIFICADOS DIGITALES)')){
            ...
            enviaIndividual(arr_ids, solo_links);
        }
    }
    function enviaIndividual(arr_ids, solo_links) {
        if(arr_ids.length>0 && sw_envios){
            ...
            if(item!=='0'){
                ...
                $.ajax({
                        ...
                        data: {id_participante: item, keyaccess: '5rw4t6gd1', id_administrador: '<?php echo administrador('id'); ?>', solo_links: solo_links},
                        type: 'POST',
                        dataType: 'html',
                        success: function (data) {

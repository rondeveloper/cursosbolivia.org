...
if (isset($_SESSION['login_esf_captcha']) && isset($_POST['pagina'])) {
    ...
    $resultado_empresas = query("SELECT correo_empresa,clave FROM empresas WHERE nombre_empresa $operador_uno 'consultor' AND estado='0' ORDER BY id DESC LIMIT " . (($vista - 1) * 20) . ",20")or die(mysqli_error($mysqli));
    while($empresa = fetch($resultado_empresas)){
    ...
    echo "Envios realizados Correctamente! <img src='".$dominio_www."contenido/imagenes/images/bien.png' style='width:25px;'>";
} else {
    echo "Denegado!";
}
?>

...
(select sum(inn_cpr.monto_deposito) from cursos_proceso_registro inn_cpr inner join cursos_participantes inn_cp on inn_cpr.id=inn_cp.id_proceso_registro where inn_cp.id_curso=c.id and inn_cp.estado=1 and inn_cp.id_modo_pago IN (4,3))dr_total_pago_banco,
(select sum(inn_cpr.monto_deposito) from cursos_proceso_registro inn_cpr inner join cursos_participantes inn_cp on inn_cpr.id=inn_cp.id_proceso_registro where inn_cp.id_curso=c.id and inn_cp.estado=1 and inn_cp.id_modo_pago IN (1))dr_total_pago_efectivo,
(select sum(inn_cpr.monto_deposito) from cursos_proceso_registro inn_cpr inner join cursos_participantes inn_cp on inn_cpr.id=inn_cp.id_proceso_registro where inn_cp.id_curso=c.id and inn_cp.estado=1 and inn_cp.id_modo_pago IN (5))dr_total_pago_tigomoney,
(select sum(inn_cpr.monto_deposito) from cursos_proceso_registro inn_cpr inner join cursos_participantes inn_cp on inn_cpr.id=inn_cp.id_proceso_registro where inn_cp.id_curso=c.id and inn_cp.estado=1 and inn_cp.id_modo_pago NOT IN (4,3,1,5))dr_total_pago_otros,
...
<div class="panel-footer text-center">
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;
<!--    <button class="btn btn-danger active" onclick="export_reporte('pdf');">PDF</button>
    &nbsp;&nbsp;&nbsp;&nbsp;-->
...
<script>
    function export_reporte(formato) {
        window.open('<?php echo $dominio; ?>pages/ajax/ajax.impresion.estadisticas-cursos.exportar-reporte.php?' + formato + '=true&data=<?php echo $postdata_exort; ?>', 'popup', 'width=700,height=500');
    }
</script>

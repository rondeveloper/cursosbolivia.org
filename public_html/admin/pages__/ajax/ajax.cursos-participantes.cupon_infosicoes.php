                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if (strlen($rqcp2['celular']) == 8) {
                                ...
                                $url_registro_infosiscon = 'https://infosiscon.com/registro.html';
                                $msj_fecha_limite_registro = fecha_final_cupon($id_cupon);
                                $text_wap = str_replace(' ', '%20', str_replace('__', '%0A', 'Hola ' . $rqcp2['nombres'] . ' ' . $rqcp2['apellidos'] . ',__Puedes descargar tu cupón Infosiscon en el siguiente enlace:__' . $url_desc_cupon.'__ __Debe registrarse en:__'.$url_registro_infosiscon.'__ __Fecha límite de registro: '.$msj_fecha_limite_registro));
                            ?>
...
function obtiene_cupones($id_paquete, $duracion, $fecha_expiracion, $cnt_participantes){
    $cont = file_get_contents("https://www.infosiscon.com/contenido/paginas/procesos/externos/webservice.cursosbo.cupones.php?id_paquete=$id_paquete&duracion=$duracion&fecha_expiracion=$fecha_expiracion&cnt_participantes=$cnt_participantes");
    return $cont;
}

function fecha_final_cupon($id_cupon){
    $rqdcp1 = query("SELECT fecha_expiracion FROM cursos_cupones_infosicoes WHERE id='$id_cupon' ORDER BY id DESC limit 1 ");
    $rqdcp2 = fetch($rqdcp1);
    $d_day = date("d", strtotime("+1 month", strtotime($rqdcp2['fecha_expiracion'])));
    $d_month = date("m", strtotime("+1 month", strtotime($rqdcp2['fecha_expiracion'])));
    $d_year = date("Y", strtotime("+1 month", strtotime($rqdcp2['fecha_expiracion'])));

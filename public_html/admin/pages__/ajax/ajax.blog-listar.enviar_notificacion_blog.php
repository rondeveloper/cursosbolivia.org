...
switch ($modo_envio) {
    case 'usuariosreg':
        ...
        while ($rqturap2 = fetch($rqtur1)) {
            ...
            if((int)$rqder2['total']==0){
                ...
                SISTsendEmail($correo, $asunto, $cont_preunsubscribe . $contenido_correo, false);
                $cnt_env++;
                if ($cnt_env == 1) {
                    //SISTsendEmail("brayan.desteco@gmail.com", $asunto, $cont_preunsubscribe . $contenido_correo . $cont_unsubscribe, false);echo "TEST ENVIADO";exit;
                }
            }
        }
    ...
    case 'anterpart':
        ...
        while ($rqturap2 = fetch($rqturap1)) {
            ...
            if((int)$rqder2['total']==0){
                ...
                $cnt_env++;
                if ($cnt_env == 1) {
                    //*SISTsendEmail("brayan.desteco@gmail.com", $asunto, $cont_preunsubscribe . $contenido_correo . $cont_unsubscribe, false);
                }
            }
        }
    ...
    case 'partnoasist':
        ...
        while ($rqturap2 = fetch($rqturapn1)) {
            ...
            $contenido_correo = str_replace(array('[REMM-CORREO]','[REMM-UNSUB]'),array($correo,$enlace_dar_baja_notificaciones),$contenido_correo);
            query("INSERT INTO cursos_rel_notifcuremail (id_curso,email) VALUES ('$id_curso','$correo') ");
            SISTsendEmail($correo, $asunto, $contenido_correo,false);
            $cnt_env++;
            if ($cnt_env == 1) {
                //*SISTsendEmail("brayan.desteco@gmail.com", $asunto, $contenido_correo . $cont_unsubscribe,false);
            }
        }
        logcursos_AUX('Notificacion de curso [correo][participantes anteriores][' . $cnt_env . ' envios]', 'notificacion-curso', 'curso', $id_curso);
        echo "<b class='btn btn-info active'>ENVIOS REALIZADOS [$cnt_env]</b>";

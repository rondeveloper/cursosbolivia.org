                ...
                <div class="table-responsive">
                    <table class="table users-table table-condensed table-hover table-striped table-bordered">
                        ...
                        <tbody>
                            <?php
                            $cnt = 1;
                            while ($noticia = fetch($resultado1)) {
                                    ?>
                                    <tr>
                                        ...
                                        <td class="visible-lgNOT">
                                            ...
                                            if($noticia['imagen']!==''){
                                            ?>
                                                <img src="<?php echo $dominio_www."contenido/imagenes/noticias/" . $noticia['imagen']; ?>" style="height:70px;width:70px;overflow:hidden;border-radius: 7px;opacity: .8;"/>
                                            <?php
                                            }else{
                                                echo "---";
                                            }
                                        ...
                                        <td class="visible-lgNOT">
                                            <?php
                                            /* url */
                                            $url_corta = $dominio.'cursos-en-' . $noticia['titulo_identificador'] . '.html';
...
<script>
    function notificar_noticia(id_noticia) {
        ...
        $.ajax({
            url: 'pages/ajax/ajax.noticias-listar.notificar_noticia.php',
...
<script>
    function actualiza_segmento_de_notificacion() {
        ...
        $.ajax({
            url: 'pages/ajax/ajax.noticias-listar.actualiza_segmento_de_notificacion.php',
            data: arraydata,
            type: 'POST',
            dataType: 'html',
            success: function(data) {
...
<script>
    function enviar_notificacion_noticia(dat, bloque) {
        ...
        var arraydata = $("#form-segmento-notificacion").serialize();
        $.ajax({
            url: '<?php echo $dominio_www; ?>pages/ajax/ajax.noticias-listar.enviar_notificacion_noticia.php?modenv=' + dat + '&bloque=' + bloque + '&ahdmd=<?php echo base64_encode($id_administrador); ?>&title=' + encodeURI(title),
            data: arraydata,
            type: 'POST',
            dataType: 'html',
            success: function(data) {
...
<script>
    function historial_noticia(id_noticia) {
        ...
        $.ajax({
            url: 'pages/ajax/ajax.noticias-listar.historial_noticia.php',
            data: {id_noticia: id_noticia},
            type: 'POST',
            dataType: 'html',
            success: function(data) {

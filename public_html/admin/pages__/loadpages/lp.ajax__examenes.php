...
if (isset_post('delete-course')) {
    if ($nivel_administrador == '1' || isset_organizador()) {
        ...
        query("DELETE FROM cursos_examenes_generales WHERE id='$id_curso_delete' ORDER BY id DESC limit 1 ");
        query("DELETE FROM cursos_rel_cursoonlinecourse WHERE id_onlinecourse='$id_curso_delete' ORDER BY id DESC ");
        //query("UPDATE cursos_examenes_generales SET estado='0' WHERE id='$id_curso_delete' ORDER BY id DESC ");
...
/* registros */
$resultado1 = query("SELECT * FROM cursos_examenes_generales c WHERE 1 $qr_busqueda $qr_estado ORDER BY id DESC LIMIT $start,$registros_a_mostrar");
$resultado2 = query("SELECT count(*) AS total FROM cursos_examenes_generales WHERE 1 $qr_busqueda $qr_organizador $qr_estado $qr_departamento ");
...
<div class="row">
    <div class="col-mod-12">
        ...
        <div class="form-group hiddn-minibar pull-right hidden-sm">
            <a <?php echo loadpage('examenes-crear'); ?> class='btn btn-success active'> <i class='fa fa-plus'></i> AGREGAR EXAMEN</a>
        </div>
        <h3 class="page-header"> EXAMENES GENERALES <i class="fa fa-info-circle animated bounceInDown show-info"></i> </h3>
                    ...
                    <table class="table users-table table-condensed table-hover table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="visible-lgNOT" style="font-size:10pt;">Examen</th>
                        ...
                        <tbody>
                            <?php
                            while ($examen = fetch($resultado1)) {
                                $tr_class = '';
                                /* curso suspendido */
                                if ($examen['sw_suspendido'] == 1) {
                                ...
                                <tr class="<?php echo $tr_class; ?>">
                                    <td class="visible-lgNOT">
                                        ...
                                        <br/>
                                        <br/>
                                        <b class="btn btn-default" onclick="historial_curso('<?php echo $examen['id']; ?>');" data-toggle="modal" data-target="#MODAL-historial_curso"><i class="fa fa-list"></i></b>
                                    </td>
                                    <td class="visible-lgNOT">
                                        <?php
                                        echo "<span style='font-size:14pt;'>" . ($examen['titulo']) . "</span>";
                                        echo "<br/>";
                                        echo "<i style='color:gray;'>URL DE ACCESO AL EXAMEN</i>";
                                        echo "<br/>";
                                        if ($nivel_administrador == '1') {
                                            /* url_ingreso */
                                            $url_corta = $dominio_plataforma.'examen/' . $examen['id'].'/' . md5('ysdy1641'.md5($examen['id'])).'.html';
                                            echo "<input type='text' class='form-control' value='" . $url_corta . "'/>";
                                        }
                                        echo "<span style='color:gray;font-size:8pt;' class='pull-right'>ID de examen: " . $examen['id'] . "</span>";                                        
                                        ?>
                                    </td>
                                    <td class="visible-lgNOT" id="td-estado-<?php echo $examen['id']; ?>">
                                        <?php
                                        if ($examen['estado'] !== '3') {
                                            ...
                                            if ($examen['estado'] == '1') {
                                                ...
                                                if (acceso_cod('adm-cursos-estado')) {
                                                    ...
                                                    <br/>
                                                    <i class="btn btn-xs btn-default btn-block" onclick="cambiar_estado_curso('<?php echo $examen['id']; ?>', 'desactivado');">Desactivado</i>
                                                    <?php
                                                }
                                            } else {
                                                ?>
                                                ...
                                                if (acceso_cod('adm-cursos-estado')) {
                                                    ...
                                                    <br/><br/><i style="font-size:7pt;color:gray;">Cambiar:</i>
                                                    <br/>
                                                    <i class="btn btn-xs btn-default btn-block" onclick="cambiar_estado_curso('<?php echo $examen['id']; ?>', 'activado');">Activado</i>
                                                    <?php
                                                }
                                            }
                                        }
                                    ...
                                    <td class="visible-lgNOT" style="width:120px;">
                                        <?php
                                        if ($examen['estado'] !== '3') {
                                            if ($nivel_administrador == '1' || $examen['estado'] == '1' || $examen['estado'] == '2') {
                                                ?>
                                                <a href="<?php echo $url_corta; ?>" target="_blank" class="btn btn-xs btn-default btn-block" style="color: #0089b5;">
                                                    <i class='fa fa-eye'></i> IR AL LINK
                                                </a>
                                                <a href="examenes-preguntas/<?php echo $examen['id']; ?>.adm" class="btn btn-xs btn-default btn-block" style="color: #0089b5;">
                                                    <i class='fa fa-copy'></i> PREGUNTAS
                                                </a>
                                                <a href="examenes-intentos/<?php echo $examen['id']; ?>.adm" class="btn btn-xs btn-default btn-block" style="color: #0089b5;">
                                                    <i class='fa fa-copy'></i> PARTICIPANTES
                                                </a>
                                                <?php
                                            }
                                        }
...
<script>
    function cambiar_estado_curso(id_examen, estado) {
        $("#td-estado-" + id_examen).html("Actualizando...");
        $.ajax({
            url: 'pages/ajax/ajax.examenes.cambiar_estado_curso.php',
            data: {id_examen: id_examen, estado: estado},
            type: 'POST',
            dataType: 'html',
            success: function(data) {
                $("#td-estado-" + id_examen).html(data);
            }
        });
    }
</script>
